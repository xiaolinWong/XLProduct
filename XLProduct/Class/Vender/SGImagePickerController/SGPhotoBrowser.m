...
- (void)panScale:(UIPanGestureRecognizer *)panScale{
    
    CGPoint translation = [panScale translationInView:self];
    panScale.view.center = CGPointMake(panScale.view.center.x + translation.x,
    panScale.view.center.y + translation.y);
    [panScale setTranslation:CGPointZero inView:self];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    if (self.browser.isImage) {
        if (self.browser.isImageUrl){
            return self.browser.imageUrlArr.count;
        }
        return self.browser.imageArr.count;
    }else{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.browser.isImage) {
        //        NSLog(@">>>>%@",self.browser.imageArr);
        if (self.browser.isImageUrl) {
            UIImage *image=[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.browser.imageUrlArr[indexPath.item]]] scale:1];
            
            //            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:self.browser.imageUrlArr[indexPath.item]] placeholderImage:PlaceholderImage];
            ...
            UIImage *image=self.browser.imageArr[indexPath.item];
            cell.imageView.image = image;
            CGFloat scale = image.size.height / image.size.width;
            cell.imageView.center = CGPointMake(SG_WIDTH*0.5, SG_HEIGHT *0.5);
            cell.imageView.bounds = CGRectMake(0, 0, SG_WIDTH, SG_WIDTH *scale);
...
- (void)setCurrentIndex:(NSInteger)currentIndex{
    ...
    if (self.isImage) {
        if (self.isImageUrl) {
            self.indexLabel.text = [NSString stringWithFormat:@"%ld/%ld",self.currentIndex + 1,self.imageUrlArr.count];
        }else{
            self.indexLabel.text = [NSString stringWithFormat:@"%ld/%ld",self.currentIndex + 1,self.imageArr.count]; ;
        }
